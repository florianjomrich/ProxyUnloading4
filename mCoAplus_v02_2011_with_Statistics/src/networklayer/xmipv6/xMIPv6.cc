	...
	*/
	...
	if (validBUMessage)
	{
		...
		if (!mCoA->getProhibited()){ //
			...
			//if a Flow Binding Update Control message did not arrive properly at HA or CN
			if(rt6->isHomeAgent() || (!rt6->isHomeAgent() && !rt6->isMobileNode() && !rt6->isRouter()) ){
			    //complete your own Flow Binding Table and through this also the CNs
			                 FlowBindingUpdate* newFlowBindingUpdateToSend = new FlowBindingUpdate();
			                    newFlowBindingUpdateToSend->setHomeAddress(bu->getHomeAddressMN().str().c_str());
			                    newFlowBindingUpdateToSend->setNewCoAdress(CoA.str().c_str());
